...
float SimpsonSimple1_3(float *a, float *b, int funcion);
...
{ int metodo, funcion, opcion = 1 ; float a,b,n;
	...
	do{
		printf("  [1] Simpson 1/3 simple \n");
        printf("  [2] Simpson 1/3 Multiple\n");
        printf("  [3] Salir\n");
        ...
        if (funcion != 4)
        {
            switch(metodo)
            {
                case 1: printf("Resultado: %f\n\n", SimpsonSimple1_3(&a,&b,funcion));
                        printf("Error: %f \n", Error(a,b,1,funcion)); // cambiar tipo de error
                        break;
                case 2: printf("Resultado: %f\n\n", TrapecioMultiple(&a,&b, &n, funcion));
                        break;
                case 4: opcion = 0;
...
{ float resultado,h;
	...
	resultado = (Funcion(*a,funcion) + 4*Funcion(*a+h,funcion) + Funcion(*b,funcion))*h/3;
